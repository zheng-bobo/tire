...
// Handle is a function that can be registered to a route
type Handle func()
...
func (n *node) incrementChildPrio(pos int) int {
	...
	if newPos != pos {
		n.indices = n.indices[:newPos] + // unchanged prefix. might be empty
			...
			n.indices[newPos:pos] + n.indices[pos+1:] // reset withput char at 'pos'
	}
	return newPos
}
	...
	walk:
		for {
			...
			if i < len(n.path) {
				...
				// Update naxParams (max of all children)
